    ...
    var body: some View {
        Group {
            if tokenPassed == nil {
                ...
                GettingStartedScrollView(users: agoraManager.allUsers, agoraEngine: agoraManager.agoraEngine)
            } else {
            ...
            Task {
                ...
                guard !channelId.isEmpty, let token = try? await fetchToken(
                    from: self.tokenUrl, channel: self.channelId,
                    role: .broadcaster, userId: 0
                ) else {
                    tokenPassed = false
                    return
                }
                tokenPassed = true
                agoraManager.agoraEngine.joinChannel(byToken: token, channelId: channelId, info: nil, uid: 0)
            }
        }.onDisappear {
